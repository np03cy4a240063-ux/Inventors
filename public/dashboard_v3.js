...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Respect low stock alerts notification preference from profile settings
    ...
    const lowStock = showLowStockAlerts
        ? productsArr.filter(p => p && Number(p.stock) > 0 && Number(p.stock) <= Number(p.min))
        : [];
