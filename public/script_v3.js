...
document.addEventListener('DOMContentLoaded', () => {
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    // Clear any data cached from a previous account before storing new user
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async (e) => {
            ...
            showLogoutConfirm(API_BASE);
        });
    }

    function showLogoutConfirm(apiBase) {
        ...
        overlay.innerHTML = `
            <div style="background:#fff;border-radius:20px;padding:40px 36px;
                ...
                <div style="display:flex;gap:12px;justify-content:center;">
                    <button id="_logoutCancel" style="flex:1;padding:12px;border:1.5px solid #E2E8F0;
                        border-radius:12px;background:#fff;color:#475569;font-weight:700;
                        font-size:0.9rem;cursor:pointer;font-family:inherit;">Cancel</button>
                    <button id="_logoutConfirm" style="flex:1;padding:12px;border:none;
                        border-radius:12px;background:#DC2626;color:#fff;font-weight:700;
                        font-size:0.9rem;cursor:pointer;font-family:inherit;">Yes, Log Out</button>
                </div>
            </div>`;
        ...
        document.getElementById('_logoutConfirm').addEventListener('click', async () => {
            document.getElementById('_logoutConfirm').innerHTML =
    ...
    toggleIcons.forEach(eye => {
        eye.addEventListener('click', (e) => {
            e.stopPropagation();
            const wrapper = eye.closest('.password-field') || eye.parentElement;
            const input = wrapper ? wrapper.querySelector('input') : null;
            if (!input) return;
            const isHidden = input.type === 'password';
            input.type = isHidden ? 'text' : 'password';
            eye.classList.remove('fa-eye', 'fa-eye-slash');
            eye.classList.add(isHidden ? 'fa-eye' : 'fa-eye-slash');
        });
    });
});
